      ...
      detailSections: [
        ...
        },
        {
          ...
          items: [
            ...
            "AI 서버는 Stateless로 설계해 요청마다 문맥과 사진 URL만 받고, 외부 AI는 연동 모듈 뒤에 격리해 변경·장애가 내부로 번지지 않게 했습니다",
          ],
          highlights: [
            "핵심 서비스의 응답성",
            "백엔드(NestJS)",
        ...
        },
        {
          title: "무슨 일을 맡았나",
          items: [
            "창업팀에서 AI 파트를 혼자 맡아, 실제 사용자에게 나가는 기능을 처음부터 끝까지 만들었습니다",
            "식단 분석 AI 서버는 설계·구현·배포·보안까지, 외주 챗봇은 검증부터 연동·앱 화면까지 담당했습니다",
          ],
          highlights: [
            "AI 파트를 혼자",
            "처음부터 끝까지",
          ...
          table: {
            headers: ["맡은 영역", "무엇을 만들었나", "규모"],
            rows: [
              ["AI 서버 (FastAPI)", "사진 분석 · AI 문구 생성 · SSRF 방어 · 운영 안정화", "약 2,400줄 · 테스트 105개"],
              ["챗봇 연동 (NestJS)", "벤더 프록시 · 동의 게이트 · 응급 가드레일 · 음성 변환", "챗봇 41개 포함 테스트 377개"],
              ["검증 도구", "계약 하네스 · AI 검증 하네스 · 지표 측정 · 부하 테스트", "4종 직접 제작"],
              ["설계 문서", "의사결정 근거와 실패 기록", "15편 · 약 380KB"],
            ],
          },
        },
        {
          ...
          items: [
            "어르신이 하는 일은 사진 한 장 올리기가 전부입니다. 음식·양·영양소는 전부 이 서버가 계산합니다",
            "자체 모델(SAM + Food-101 한식 파인튜닝)도 만들어 뒀지만, 월 44만 건까지는 API가 싸다는 실측으로 GPT Vision을 택하고 언제든 교체 가능하게 설계했습니다",
          ],
          highlights: [
            "사진 한 장 올리기가 전부",
            "전부 이 서버가 계산",
            "자체 모델(SAM + Food-101 한식 파인튜닝)",
            "월 44만 건까지는 API가 싸다는 실측",
            "GPT Vision",
            "언제든 교체 가능하게 설계",
          ],
          docs: [
        ...
        },
        {
          ...
          title: "건강 상담 챗봇 연동",
          items: [
            "어르신이 말로 건강 상태를 얘기하면 챗봇이 되묻고, 그 내용이 정리돼 보호자에게 전달됩니다. 챗 엔진은 외주로 납품된 것이라 붙이기 전에 직접 검증했습니다",
            "흉통·호흡곤란을 함께 말해도 \"계속되면 상담받아 보세요\"라고 답하는 것을 발견해, 119 안내는 우리 서버가 판단해 얹습니다",
          ],
          highlights: [
            "말로 건강 상태를 얘기하면",
            "정리돼 보호자에게 전달",
        ...
        },
        {
          ...
          title: "AI를 어떻게 검증했나",
          items: [
            "LLM은 같은 입력에 같은 답을 주지 않습니다. 그래서 값이 아니라 성질을 검증했습니다 — 불변식·메타모픽·적대적 입력",
            "사람이 다칠 수 있는 판정은 정답 세트 46건으로 재현율을 쟀습니다. 처음 재보니 45.8%였습니다",
          ],
          highlights: [
            "같은 입력에 같은 답을 주지 않습니다",
            "값이 아니라 성질을 검증",
            "불변식·메타모픽·적대적 입력",
            "정답 세트 46건",
            "재현율",
        ...
        {
          title: "트러블슈팅 — 실제로 터진 것들",
          items: [
            "지표는 전부 초록인데 실제로는 틀려 있던 두 건이 있었습니다. 테스트 366개가 통과한 채로 가짜 챗봇이 어르신에게 응답했고, 라인 커버리지 100%인 응급 안전장치가 음성에서 4건 중 3건을 놓쳤습니다",
            "둘 다 자동 검증이 아니라 직접 써 보고 나서 드러났습니다. 그래서 검증 계층 맨 앞에 배포 환경과 실사용 경로를 확인하는 단계를 넣었습니다",
          ],
          highlights: [
            "지표는 전부 초록인데 실제로는 틀려",
            "테스트 366개가 통과한 채로",
        ...
        },
        {
          title: "경험을 통해 배운 점",
          items: [
            "AI 시대의 병목은 코드 작성이 아니라 무엇을 검증할지 정하는 일이라는 것 — 오래 걸린 건 전부 판단이었지 타이핑이 아니었다",
            "문서는 가설이고 실측이 사실이라는 것 — 연동하기 전에 직접 호출해 확인한다",
            "기본값은 \"동작하지만 틀림\"이 아니라 \"동작하지 않음\"이어야 한다는 것",
            "막지 못한 것을 적어야 한다는 것 — 안 적으면 다음 사람은 막혀 있다고 믿는다",
          ],
          highlights: [
            "무엇을 검증할지 정하는 일",
            "문서는 가설이고 실측이 사실",
    ...
    },
    {
      ...
      detailSections: [
        ...
        },
        {
          ...
          layout: "paragraphs",
          media: {
            src: "/assets/experiences/ai-agent/data-camp.png",
            alt: "2024 데이터 크리에이터 캠프 온라인 연수원",
            width: 630,
            height: 396,
            maxWidth: 220,
            placement: "left",
