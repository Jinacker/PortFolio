      ...
      detailSections: [
        {
          title: "끼니톡은 어떤 서비스인가",
          items: [
            "혼자 사시는 어르신의 식사 불균형·복약 누락·건강 이상을 가족이 제때 알아차리기 어렵습니다",
            "끼니톡은 어르신 앱과 보호자 앱을 연결해 식사·복약·건강 상태를 자연스럽게 공유하는 서비스입니다",
            "기존 건강관리 서비스처럼 직접 검색하고 기록하는 대신, 사진 한 장과 말 한마디만으로 정보를 남길 수 있도록 AI가 입력 과정을 대신합니다",
            "어르신은 쉽게 사용하고, 가족은 멀리서도 자연스럽게 확인할 수 있게 하는 것이 끼니톡의 핵심입니다",
          ],
          highlights: [
            "식사 불균형·복약 누락·건강 이상을 가족이 제때 알아차리기 어렵습니다",
            "어르신 앱과 보호자 앱을 연결해 식사·복약·건강 상태를 자연스럽게 공유하는 서비스",
            "사진 한 장과 말 한마디만으로 정보를 남길 수 있도록 AI가 입력 과정을 대신합니다",
            "어르신은 쉽게 사용하고, 가족은 멀리서도 자연스럽게 확인할 수 있게 하는 것",
            "Vision이 음식·면적·영양소를 분석",
            "LLM이 약별 주의 음식 생성",
          ...
          table: {
            ...
            rows: [
              ["식사", "사진 한 장", "Vision이 음식·면적·영양소를 분석, 부족분 산출"],
              ["복약", "알림 확인", "LLM이 약별 주의 음식 생성, 식단과 충돌 매칭"],
        ...
        },
        {
          title: "아키텍처",
          items: [
            "핵심 서비스의 응답성을 유지하기 위해 백엔드(NestJS)와 고부하 AI 연산을 담당하는 AI 서버(FastAPI)를 분리했습니다",
            "독립적인 확장과 데이터 저장 최소화를 위해 AI 서버를 Stateless로 설계해 필요한 문맥과 사진 URL만 요청 단위로 전달합니다",
            "외부 AI 변경·장애가 내부 로직에 영향을 주지 않도록 GPT Vision·챗 엔진을 연동 모듈 뒤에 격리하고 요청·응답과 오류를 정규화했습니다",
          ],
          highlights: [
            "핵심 서비스의 응답성",
            "백엔드(NestJS)",
            "AI 서버(FastAPI)를 분리",
            "AI 서버를 Stateless로 설계",
            "요청 단위로 전달",
            "연동 모듈 뒤에 격리",
            "요청·응답과 오류를 정규화",
          ],
        ...
        },
        {
          title: "무슨 일을 맡았나",
          items: [
            "창업팀에서 AI 파트 전체를 전담하며 실제 서비스에 들어가는 기능 개발",
            "식단 사진 분석 AI 서버(FastAPI)를 설계부터 구현·운영·보안까지 처음부터 구축",
            "외부 벤더 건강상담 챗봇 연동과 응급 가드레일, 음성(STT/TTS) 흐름 담당",
            "음성 건강대화 등 AI 관련 앱 화면 개발, 녹음·무음 감지 UX 구현",
            "불변식·메타모픽·적대적 입력으로 비결정적 AI를 검증하는 체계 설계·실측",
          ],
          highlights: [
            "AI 파트 전체를 전담",
            "식단 사진 분석 AI 서버(FastAPI)",
            "설계부터 구현·운영·보안까지",
            "건강상담 챗봇 연동",
            "응급 가드레일",
            "음성(STT/TTS)",
        ...
        },
        {
          ...
          items: [],
          docs: [
            { href: "/mds/kkinitalk/impl-1-pipeline.md", label: "식단 분석 파이프라인 — 교체 가능성이 설계의 중심" },
            { href: "/mds/kkinitalk/impl-2-ops.md", label: "AI 서버 운영 설계 — 추론 서버가 죽는 네 가지 방식을 막기" },
            { href: "/mds/kkinitalk/impl-3-llm.md", label: "LLM 연동 계층 — 비결정 출력을 계약으로 묶기" },
            { href: "/mds/kkinitalk/impl-4-ssrf.md", label: "이미지 수집 계층 — SSRF 방어와 프라이버시" },
            { href: "/mds/kkinitalk/impl-5-testing.md", label: "2층 검증 체계 — 서로 다른 결함을 잡는 두 개의 그물" },
            { href: "/mds/kkinitalk/impl-6-chatbot.md", label: "외부 LLM 챗봇 연동 — 외주 업체가 만든 AI를 우리 얼굴로 내보내기" },
          ],
        },
        {
          title: "AI 모델 검증",
          items: [],
          docs: [
            { href: "/mds/kkinitalk/verify-0-approach.md", label: "왜 일반 테스트가 성립하지 않는가" },
            { href: "/mds/kkinitalk/verify-1-harness.md", label: "챗봇 LLM 검증 하네스 — 3축 + 운영 지표" },
            { href: "/mds/kkinitalk/verify-2-adversarial.md", label: "적대적 입력이 찾은 안전 결함 — 계약이 비어 있었다는 발견" },
            { href: "/mds/kkinitalk/verify-3-metrics.md", label: "응급 가드 성능 지표 — precision · recall · F1" },
            { href: "/mds/kkinitalk/verify-4-vision.md", label: "식단 분석(Vision) 일관성 측정 — 같은 사진 5회에 5가지 답" },
            { href: "/mds/kkinitalk/verify-5-load.md", label: "부하·동시성 — 코드에만 있던 안전장치를 처음으로 증명" },
            { href: "/mds/kkinitalk/verify-6-layers.md", label: "검증 계층 — 무엇이 무엇을 못 잡는가" },
          ],
        },
        {
          title: "트러블슈팅",
          items: [],
          docs: [
            { href: "/mds/kkinitalk/ts-1-cost.md", label: "GPT Vision 건당 비용 12배 절감" },
            { href: "/mds/kkinitalk/ts-2-latency.md", label: "LLM 응답 3~4초 → 1.5~2.5초" },
            { href: "/mds/kkinitalk/ts-3-label-space.md", label: "한식이 전부 \"영양 정보 미등록\"으로 떨어지던 문제" },
            { href: "/mds/kkinitalk/ts-4-label-wobble.md", label: "라벨이 매번 달라 영양소가 흔들린다 — 모델을 못 고칠 때 남는 레버" },
            { href: "/mds/kkinitalk/ts-5-stt.md", label: "벤더 STT가 음성을 못 읽는다 — 두 번의 실험으로 원인 좁히기" },
            { href: "/mds/kkinitalk/ts-6-guard.md", label: "응급 가드가 음성 경로에서 뚫렸다 — AI 안전성 검증의 사각" },
            { href: "/mds/kkinitalk/ts-7-fake.md", label: "운영에서 가짜 AI가 사용자에게 응답했다" },
            { href: "/mds/kkinitalk/ts-8-agent.md", label: "AI 에이전트로 개발하면서 생긴 검증 공백" },
          ],
        },
        {
          title: "경험을 통해 배운 점",
          items: [
            ...
            "문서는 가설이고 실측이 사실이라는 것 — 붙이기 전에 찔러 본다",
            "기본값은 \"동작하지만 틀림\"이 아니라 \"동작하지 않음\"이어야 한다는 것",
            "판정 결과가 아니라 판정 근거를 단언해야 한다는 것 — 결과만 보면 이유가 틀려도 통과한다",
            "비결정적인 것을 판정 대상으로 삼지 않는다는 것 — LLM 출력이 아니라 사용자 입력을 본다",
