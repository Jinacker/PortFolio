    ...
    },
    {
      ...
      detailSections: [
        ...
        },
        {
          title: "핵심 구현",
          items: [
            "원래 계획은 자체 모델 — SAM 세그멘테이션 + Hugging Face Food-101(ViT) 분류기를 직접 한식 데이터로 파인튜닝해 직접 운영하는 것까지 준비했습니다(코드 완성)",
            "그런데 GPU 서버 비용(월 15만~38만 원)이 커서 우선 GPT Vision으로 운영하며 비용을 실측 — 손익분기 월 약 44만 건 전까지는 API가 유리하다는 결론을 얻었습니다",
            "그래서 아래 구현 전부가 \"모델을 언제든 갈아끼울 수 있는 구조\"를 전제로 설계됐습니다",
          ],
          highlights: [
            "자체 모델",
            "SAM 세그멘테이션 + Hugging Face Food-101(ViT)",
            "한식 데이터로 파인튜닝",
            "GPU 서버 비용(월 15만~38만 원)",
            "GPT Vision",
            "손익분기 월 약 44만 건",
            "모델을 언제든 갈아끼울 수 있는 구조",
          ],
          docs: [
            ...
            { href: "/mds/kkinitalk/impl-5-chatbot.md", label: "외부 LLM 챗봇 연동 — 외주 업체가 만든 AI를 우리 얼굴로 내보내기" },
          ],
        },
        {
          title: "AI 모델 검증",
          items: [],
          docs: [
            { href: "/mds/kkinitalk/verify-1-method.md", label: "비결정 AI를 어떻게 검증하는가 — 값 대신 성질" },
            { href: "/mds/kkinitalk/verify-2-emergency.md", label: "적대적 입력이 찾은 응급 결함 — recall 45.8% → 100%" },
            { href: "/mds/kkinitalk/verify-3-vision.md", label: "Vision 일관성 측정 — 같은 사진 5회에 5가지 답" },
            { href: "/mds/kkinitalk/verify-4-load.md", label: "부하·동시성 — 코드에만 있던 안전장치를 처음으로 증명" },
          ],
        },
        {
          title: "트러블슈팅",
          items: [],
          docs: [
            { href: "/mds/kkinitalk/ts-1-cost.md", label: "GPT 비용 12배·응답 2배 — 측정이 찾은 최적화" },
            { href: "/mds/kkinitalk/ts-2-label.md", label: "한식이 전부 \"영양 미등록\" — 라벨 공간과의 싸움" },
            { href: "/mds/kkinitalk/ts-3-stt.md", label: "벤더 STT가 음성을 못 읽는다 — 두 번의 실험으로 원인 좁히기" },
            { href: "/mds/kkinitalk/ts-4-guard.md", label: "응급 가드가 음성 경로에서 뚫렸다 — AI 안전성 검증의 사각" },
            { href: "/mds/kkinitalk/ts-5-fake.md", label: "운영에서 가짜 AI가 응답했다 — 전부 초록불이었다" },
          ],
        },
        {
          title: "경험을 통해 배운 점",
