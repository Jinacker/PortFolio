    ...
    },
    {
      ...
      detailSections: [
        ...
        },
        {
          ...
          items: [
            "지금은 AI를 제품 기능에 넣는 것뿐 아니라, 개발 과정의 생산성을 높이는 데에도 관심이 많습니다. 대학생에게 토큰 비용도 무시할 수 없는 문제라, 적은 토큰으로 긴 작업의 맥락을 유지하는 방법도 꾸준히 고민하고 있습니다.",
            "AI Agent에게 구현을 맡기더라도 방향을 정하고 결과를 검증하는 책임까지 넘기지는 않습니다. 아래 내용은 여러 프로젝트에서 직접 시행착오를 겪으며 정리한 현재의 개발 방식입니다.",
          ],
          highlights: [
            "개발 과정의 생산성을 높이는 데",
            "토큰 비용도 무시할 수 없는 문제",
            "적은 토큰으로 긴 작업의 맥락을 유지",
            "방향을 정하고 결과를 검증하는 책임",
            "직접 시행착오를 겪으며 정리한 현재의 개발 방식",
          ],
          layout: "paragraphs",
          showSubDetails: true,
        },
      ...
      subDetails: [
        {
          id: "agent-headroom",
          title: "Headroom — 토큰을 아끼고 공통 컨텍스트를 재사용",
          sections: [
            {
              title: "어떻게 활용했나",
              items: [
                "긴 작업에서 매번 전체 대화를 다시 설명하지 않도록 요구사항·설계 결정·현재 상태를 작업 문서로 정리해 공통 컨텍스트로 사용합니다.",
                "Headroom으로 에이전트가 읽어야 할 맥락을 압축하고 필요한 정보만 넘겨, 토큰 소비를 줄이면서도 작업 간 기준이 흔들리지 않게 관리합니다.",
              ],
              highlights: [
                "요구사항·설계 결정·현재 상태",
                "공통 컨텍스트",
        ...
        {
          id: "agent-multi",
          title: "멀티 에이전트 — 역할을 나누되 통합 책임은 한곳에",
          sections: [
            {
              title: "어떻게 활용했나",
              items: [
                "Claude Max 환경에서 메인 에이전트는 계획 검토·작업 연결·통합 테스트를 맡기고, 프론트엔드와 백엔드처럼 독립적인 작업은 서브 에이전트에 나눠 병렬로 진행합니다.",
                "역할을 나누는 것보다 중요한 것은 경계입니다. 공통 타입·API 계약·완료 조건을 먼저 정하고, 최종 결과는 메인 에이전트와 제가 함께 연결해 실제 화면과 요청 흐름으로 확인합니다.",
              ],
              highlights: [
                "계획 검토·작업 연결·통합 테스트",
                "서브 에이전트",
        ...
        {
          id: "agent-harness",
          title: "검증 하네스 — AI 결과를 감이 아니라 기준으로 확인",
          sections: [
            {
              title: "어떻게 활용했나",
              items: [
                "구현 전에 정상·실패 조건과 반드시 거쳐야 할 테스트 훅을 체크리스트에 넣어, 에이전트가 테스트를 생략하거나 규칙 밖의 변경을 하지 못하게 합니다.",
        ...
        {
          id: "agent-guardrails",
          title: "CLAUDE.md — 에이전트의 작업 범위와 금지선을 명시",
          sections: [
            {
              title: "운영 데이터를 잃고 세운 원칙",
              items: [
                "과거 AI에 작업을 과도하게 맡겼다가 운영 DB 데이터를 잃은 뒤, 운영 환경과 테스트 환경을 분리하고 데이터베이스 변경 권한을 제한했습니다.",
                "CLAUDE.md에 접근 가능한 범위, 금지 명령, 필수 검증 절차를 적습니다. 운영 DB 변경은 에이전트가 직접 수행하지 않고 제가 결과를 검토한 뒤 수동으로 반영합니다.",
              ],
              highlights: [
                "운영 DB 데이터를 잃은 뒤",
                "운영 환경과 테스트 환경을 분리",
                "데이터베이스 변경 권한을 제한",
                "접근 가능한 범위, 금지 명령, 필수 검증 절차",
                "검토한 뒤 수동으로 반영",
              ],
            },
          ],
        ...
        {
          id: "agent-workflow",
          title: "계획 기반 풀사이클 — 요구사항부터 직접 검증까지",
          sections: [
            {
              title: "현재 사용하는 개발 흐름",
              items: [
                "## 1. 요구사항과 계약 정리",
                "요구사항을 정리한 뒤 DB와 API 설계를 요청하고, 도메인 규칙·데이터 정합성·예외 상황을 직접 검토합니다.",
                "## 2. 전체 계획을 작은 단계로 분해",
                "처음부터 끝까지의 계획을 S0~S9처럼 나누고, 발생 가능한 문제를 검토해 순서와 범위를 조정합니다.",
                "## 3. 단계별 체크리스트로 구현",
                "각 단계마다 개발·테스트·완료 조건·필수 훅을 포함한 작업 체크리스트를 만듭니다. 메인 에이전트는 연결과 검증을, 서브 에이전트는 경계가 분명한 구현을 맡습니다.",
                "## 4. 실행 결과를 직접 확인",
                "단계가 끝날 때마다 서비스를 띄워 핵심 흐름을 직접 테스트하고, plan_result.md에 계획과 실제 결과의 차이를 기록합니다.",
                "## 5. 변경되는 요구사항을 버전으로 관리",
                "버그와 변경사항은 결합도에 따라 원래 계획에 반영하거나 별도 처리합니다. 최신안은 working.md에 유지하고, 중요한 시점은 v1·v2 문서로 남깁니다.",
              ],
              highlights: [
                "DB와 API 설계",
                "도메인 규칙·데이터 정합성·예외 상황",
                "S0~S9",
                "개발·테스트·완료 조건·필수 훅",
                "서비스를 띄워 핵심 흐름을 직접 테스트",
                "plan_result.md",
                "계획과 실제 결과의 차이",
                "working.md",
                "v1·v2 문서",
            ...
            {
              title: "아직 적용 전인 개선 아이디어",
              items: [
                "에이전트가 반복해서 만드는 실수와 버그를 gotcha 문서에 축적하고, 이를 다음 작업의 가이드라인으로 되돌리는 방법을 교수님께 배웠습니다. 아직 실제 프로젝트에 적용하지는 않았으며 다음 개발에서 실험해볼 계획입니다.",
              ],
              highlights: [
                "gotcha 문서",
                "가이드라인으로 되돌리는 방법",
                "아직 실제 프로젝트에 적용하지는 않았으며",
              ],
            },
          ],
