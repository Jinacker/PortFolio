    ...
    },
    {
      ...
      detailSections: [
        ...
        },
        {
          ...
          title: "핵심 구현 — 식단 분석 서버와 건강 상담 챗봇을 연동했습니다",
          items: [
            "식사 사진에서 음식·양·영양소를 분석하는 AI 서버를 만들고, 외부 건강 상담 챗봇을 끼니톡 앱에 연결했습니다.",
            "사진 분석 방식은 실제 호출 비용을 측정해 정했고, 벤더 API는 문서와 실제 응답을 대조한 뒤 서비스 토큰과 건강정보를 보호하는 구조로 연동했습니다.",
          ],
          highlights: [
            "음식·양·영양소를 분석하는 AI 서버",
            "실제 호출 비용을 측정해 정했고",
            "문서와 실제 응답을 대조",
            "서비스 토큰과 건강정보를 보호하는 구조",
          ],
          layout: "paragraphs",
          docs: [
        ...
        },
        {
          ...
          title: "AI 결과와 운영 안전장치를 실제 입력과 동시 요청으로 검증했습니다",
          items: [
            "같은 입력에도 답변이 달라질 수 있는 AI는 특정 문장을 비교하는 대신, 반드시 지켜야 할 응답 구조와 안전 기준을 확인했습니다.",
            "외부 챗봇은 19개 항목으로 인수 검증했고, 응급 안내는 합성 음성을 STT로 변환한 문장이 포함된 정답 세트로, 서버의 동시 실행 제한과 타임아웃은 고정 지연을 넣은 동시 요청 시나리오로 검증했습니다.",
          ],
          highlights: [
            "반드시 지켜야 할 응답 구조와 안전 기준",
            "19개 항목으로 인수 검증",
            "합성 음성을 STT로 변환한 문장이 포함된 정답 세트",
            "고정 지연을 넣은 동시 요청 시나리오",
          ],
          layout: "paragraphs",
          docs: [
            ...
            { href: "/mds/kkinitalk/verify-5-load.md", label: "동시 요청 테스트로 AI 서버의 실행 제한과 타임아웃을 검증했습니다" },
          ],
        },
        {
          title: "트러블슈팅",
          items: [
            "실기기와 배포 환경에서만 드러난 가짜 챗봇·응답 지연·안드로이드 음성 문제의 원인을 좁혀 수정했습니다.",
            "수정 뒤에는 운영에서 가짜 응답을 차단하고, 실제 AI 호출 시간과 오디오 형식 변환을 다시 확인해 같은 문제가 재발하면 드러나도록 했습니다.",
          ],
          highlights: [
            "실기기와 배포 환경에서만 드러난",
            "원인을 좁혀 수정",
          ...
          layout: "paragraphs",
          docs: [
            { href: "/mds/kkinitalk/ts-1-fake.md", label: "운영에서 개발용 가짜 챗봇이 응답하던 문제를 막았습니다" },
        ...
        },
        {
          title: "경험을 통해 배운 점",
          items: [
            "AI 기능에서는 코드를 작성하는 것보다 무엇을 검증할지 정하는 일이 더 중요하다는 점을 배웠습니다.",
            "외부 API 문서는 구현 전 가설로 두고, 실제 응답을 호출해 계약과 다른 부분을 먼저 확인하고 있습니다.",
            "운영의 기본값은 틀린 결과를 정상처럼 보여주기보다, 사용할 수 없는 상태를 명확히 드러내도록 정했습니다.",
            "현재 막지 못하는 범위도 함께 기록해, 다음 작업자가 검증이 끝난 영역으로 오해하지 않도록 했습니다.",
          ],
          highlights: [
            "무엇을 검증할지 정하는 일",
            "실제 응답을 호출해",
