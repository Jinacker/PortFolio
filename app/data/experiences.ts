  ...
  ko: [
    {
      ...
      detailSections: [
        ...
        },
        {
          ...
          items: [
            "출시 첫 달부터 실사용자가 생겼고, 앱은 강제 업데이트가 없어 구버전이 계속 서버를 호출합니다. 그래서 모든 변경의 첫 질문은 \"기존 사용자가 깨지는가\"였습니다",
            "이 제약이 설계 곳곳에 구조로 남았습니다 — DB 마이그레이션 25건 전부 additive(파괴적 변경 0건), 인증 교체 시 신·구 공존 가드, 지도 웹은 구버전용 파일을 지우지 않는 3버전 동시 서빙",
          ],
          highlights: [
            "강제 업데이트가 없어",
            "\"기존 사용자가 깨지는가\"",
            "마이그레이션 25건 전부 additive",
            "파괴적 변경 0건",
            "신·구 공존 가드",
            "3버전 동시 서빙",
          ],
        ...
        },
        {
          ...
          title: "운영 트러블슈팅 — 로그로 찾고, 고치고, 실측으로 닫는다",
          items: [
            "운영 서버 로그(Cloud Run)와 앱 에러 리포트(Sentry)를 교차 분석해 문제를 발굴하고, 고친 뒤에는 같은 지표를 다시 재서 사건을 닫았습니다",
            "대표 사건: 푸시를 보낼 때마다 복귀 유입이 rate limit에 걸리던 문제 — 발견부터 점진 발송 배포, 전/후 실측(429 28건→0건, 발송 응답 11.43초→0.18초)까지 풀사이클로 해결했습니다",
          ],
          highlights: [
            "로그(Cloud Run)와 앱 에러 리포트(Sentry)를 교차 분석",
            "같은 지표를 다시 재서 사건을 닫았습니다",
            "429 28건→0건",
            "11.43초→0.18초",
            "풀사이클",
          ],
