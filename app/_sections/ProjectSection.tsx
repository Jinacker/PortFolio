...
const competencies = [
  {
    ...
    paragraphs: [
      ...
      "실서비스 운영과 레거시 프로젝트 경험을 통해, 좋은 수정은 많은 코드를 바꾸는 것이 아니라 문제의 원인을 정확히 찾아 필요한 부분만 안정적으로 바꾸는 것이라는 기준을 갖게 되었습니다.",
    ],
  },
  {
    id: "full-cycle",
  ...
  },
  {
    ...
    paragraphs: [
      "요구사항은 계속 바뀌기 때문에, 당장의 기능 구현보다 이후의 변경을 견딜 수 있는 구조를 중요하게 생각합니다.",
      "기능이 늘어날수록 임시로 API를 덧붙이기보다, 사용자 기능과 관리자 기능의 책임을 나누고 데이터가 어떤 기준으로 변경되어야 하는지 먼저 정리합니다. 상태가 바뀌는 기능에서는 중복 처리나 예외 상황까지 함께 고려하며, 서비스의 핵심 규칙이 흔들리지 않도록 설계하려고 합니다.",
      "돈가스 지도에서는 사용자 앱과 관리자 도구가 같은 데이터를 다루더라도 서로 다른 책임을 갖도록 분리했고, 개인화 기능을 제공하면서도 인증 정보가 불필요하게 노출되지 않도록 구조를 설계했습니다.",
    ],
  },
  {
    id: "ai-spatial-value",
    ...
    paragraphs: [
      ...
      "또한 AI를 단순 코드 생성기가 아니라 설계 검토, 구현 보조, 리뷰, 오류 분석을 돕는 개발 도구로 활용하고 있습니다. 기능 구현 전 의도, 구조, 예외 상황을 문서화하고 이를 AI 컨텍스트로 활용해 작업의 방향성을 유지합니다. 빠르게 구현하되 도메인 규칙과 품질 기준은 직접 검토하는 방식으로 개발 속도와 안정성을 높이고 있습니다.",
    ],
  },
] as const;
  ...
  return (
    <SectionWatcher id="project">
      <SlideUpInView>
        ...
        <div className="mx-auto flex w-full max-w-[780px] flex-col gap-2.5 md:h-[400px] md:flex-row">
          {competencies.map(({ id, title, images, highlights, paragraphs }, index) => {
