...
| | |
|---|---|
| **발견** | Sentry의 `504` 오류 12건이 4분 동안 한 사용자에게 집중됐습니다 |
...
30일 Sentry 기록을 확인하던 중 `AxiosError: Request failed with status code 504` 12건을 발견했습니다. 오류는 모두 7월 22일 18:01~18:05 UTC, 한국 시각으로는 7월 23일 새벽 3시대의 **4분 구간에만 집중**됐습니다.

짧은 시간에 같은 오류가 반복됐기 때문에 서버가 일시적으로 요청을 처리하지 못한 상황을 먼저 의심했습니다. 하지만 같은 30일의 Cloud Run 요청 로그에는 `5xx` 응답이 없었습니다. **앱에는 504가 남았지만 애플리케이션 서버가 504를 반환한 기록은 없는 상태**였습니다.
