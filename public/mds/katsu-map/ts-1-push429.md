...
`429` 응답 42건을 시간대별로 나누자 평소에 고르게 발생한 것이 아니라 **네 시간대에만 몰려 있었습니다.** 가장 큰 구간은 7월 23일 10시의 28건이었고, 같은 시간대에 10시 47분 전체 푸시 발송이 있었습니다. 7월 14일의 3건도 전체 발송 시각과 겹쳐, **42건 중 31건이 푸시를 보낸 시간대와 일치했습니다.**

![30일간 발생한 429 응답과 전체 푸시 발송 시각을 대조한 운영 로그](/assets/experiences/katsu-map/capture-03-429-burst.svg)

가장 큰 구간에서 거부된 요청은 공지·메시지처럼 **앱을 열 때 함께 호출하는 API**였습니다. 요청 환경도 iOS 15건, Android 13건으로 실제 앱 사용과 일치했고, Sentry에서도 같은 시점에 `429` 오류가 함께 증가했습니다.
