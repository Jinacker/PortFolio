# 앱 전반의 응답 지연을 분석해 병목 구간을 개선했습니다

> **여러 API가 동시에 호출될 때 인증 과정의 반복 DB 쓰기와 작은 커넥션 풀이 함께 병목을 만들고 있었습니다. 활동 기록과 세션 갱신 빈도를 줄이고, 커넥션 풀을 확대해 처리 구조를 개선했습니다.**
...
| **지연 요인** | 인증 요청마다 활동 기록과 세션 수정이 발생했고, 인스턴스당 Prisma 커넥션 풀도 1로 설정돼 있었습니다             |
| **변경** | 활동 기록은 하루 첫 활동에만 남기고 세션 수정은 10분 간격으로 줄였으며, Prisma 커넥션 풀 크기를 1에서 5로 늘렸습니다 |
| **확인** | 변경 후 24시간 운영 로그에서 식당 목록 p50 240ms·p95 510ms를 확인했습니다                                            |
...
| **Prisma 커넥션 풀 크기 1**      | 동시에 발생한 DB 작업이 사용할 수 있는 연결이 인스턴스당 1개로 제한돼 있었습니다 |

특정 SQL 하나가 느린 것이 아니라, 한 화면에서 여러 API가 동시에 호출될 때 **각 인증 요청마다 DB 쓰기가 추가되고 그 작업들이 제한된 DB 연결을 함께 사용하는 구조를 주요 병목으로 판단했습니다.**

## 반복되던 DB 쓰기의 빈도를 줄였습니다

**활동 기록 — 요청마다 `INSERT` → KST 기준 하루 첫 활동일 때만 기록**

처음에는 인증 요청마다 추가되던 활동 기록을 제거해 DB 쓰기를 줄였습니다. 하지만 이 기록을 사용하던 **관리자 DAU 통계까지 함께 멈추는 문제**가 생겼습니다. 그래서 기록 자체를 없애는 대신 **KST 기준 하루 첫 활동일 때만 기록하도록 변경**해, 쓰기 빈도를 줄이면서 DAU 집계에 필요한 정보는 유지했습니다.

**세션 최근 사용 시각 — 요청마다 `UPDATE` → 마지막 갱신 후 10분이 지났을 때만 수정**

세션 유휴 만료 기준이 90일인 점을 고려해, **10분 정도의 기록 차이는 허용 가능한 범위로 보고 불필요한 `UPDATE` 횟수를 줄였습니다.**

## 동시에 처리할 수 있는 DB 연결을 늘렸습니다
...
변경 후 24시간 동안 같은 API의 운영 로그 23건을 확인한 결과, **p50 240ms·p95 510ms의 응답 시간을 확인했습니다.**
