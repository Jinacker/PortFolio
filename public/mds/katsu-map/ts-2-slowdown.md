...
| **확인** | 3개월 뒤 식당 목록 운영 로그에서 p50 240ms·p95 510ms를 확인했습니다                                                  |
...
| 3개월 뒤 24시간 운영 로그 23건 | **p50 240ms · p95 510ms** |

당시 개선 직후 동일한 조건의 응답 시간을 별도로 기록해두지는 않아 **정확한 전후 개선율로 계산하지 못했습니다.** 다만 3개월 뒤 같은 API의 운영 로그를 다시 확인한 결과, **p50 240ms·p95 510ms로 당시보다 안정적인 응답 상태를 확인했습니다.**
