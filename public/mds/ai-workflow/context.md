...
실제 모양은 대단할 것 없이, 새 세션의 에이전트가 이것만 읽으면 이어서 일할 수 있는 수준이면 충분합니다.

```markdown
# working.md (예시)

## 지금 기준
- 시식 기록에 "재방문 의사" 필드 포함 — 저장은 선택 입력
- 추천은 취향 파라미터 기반, 구버전 앱 호환이 최우선

## 최근 바뀐 것
- 커뮤니티 피드 정렬을 최신순으로 변경 (이전 설계는 v2.md)

## 다음 작업이 알아야 할 것
- S5 화면 연동에서 기록 응답의 mealId 사용
```
