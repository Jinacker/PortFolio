...
이 분업은 말로 하는 약속이 아니라 프로젝트 CLAUDE.md에 규칙으로 적혀 있습니다. 아래는 끼니톡 백엔드 CLAUDE.md의 실제 발췌입니다.

```markdown
- task에 적힌 내용은 순차적이나, 백엔드와 앱 코드를 모두 개발해야 하므로
  멀티 서브 에이전트를 2개 호출하여 각각 프론트와 백엔드 업무를 위임하고,
  동일한 task 파일을 보고 서로 작업 영역이 겹치지 않고 독립적이도록
  하여 작업을 요청할 것.
- 테스트, 연결 검증 시에는 메인 에이전트가 진행.
```

같은 Task 파일을 보되 영역이 겹치지 않게 나누고, 합치는 검증은 메인 한곳이 맡는다 — 이 문서에서 설명한 구조가 규약 두 줄로 압축되어 있습니다.
