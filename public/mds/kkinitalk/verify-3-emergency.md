# 건강 상담 챗봇이 놓치던 응급 표현을 찾아 보완했습니다
...
## 벤더 반영을 기다리지 않고 사용자 발화를 먼저 확인했습니다

앞선 19개 항목 검증에서 응급 상황을 알리는 발화에도 벤더 챗봇이 즉시 119를 안내하지 않는 것을 확인했습니다.
...
벤더 측에 안전 요구사항을 전달했지만, 반영을 기다리는 동안에도 응급 대응의 공백이 생기지 않도록 **우리 서버에 1차 보호장치를 추가했습니다.**

벤더 답변에 `119`가 포함됐는지를 검사하는 방식은 사용하지 않았습니다. **LLM 답변은 매번 표현이 달라질 수 있고, 벤더 호출 자체가 실패하면 안전장치도 함께 사라지기 때문입니다.**
