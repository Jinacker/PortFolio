...
- 검증 절차 맨 앞에 두 질문을 넣었다 — **검증한 커밋과 설정이 배포된 것과 같은가**, **사용자도 그 입력을 그렇게 만드는가**

두 번째 질문이 필요했던 이유는 같은 시기에 응급 안전장치도 타이핑으로만 검증해 음성에서 뚫렸기 때문이다. 검증한 경로와 실제 경로가 다르면 커버리지 숫자는 아무것도 말해 주지 않는다.
...
재발 방지 장치는 테스트로 잠갔고, 배포 후 재검증을 한 명령으로 고정해 실서버에서 확인한다.
