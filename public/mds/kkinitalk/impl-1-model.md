...
저해상도 옵션에서도 같은 사진이 비빔밥으로 인식됐고, 이미지 입력 비용은 약 12분의 1로 줄었습니다. 이후 실제 급식 사진과 반찬이 많은 한정식도 호출해 서비스 흐름과 입력 변화에 따른 결과를 별도로 확인했습니다. 다만 위 수치는 **한 장의 고정된 사진으로 옵션 차이만 비교한 결과**이므로, 저해상도에서도 모든 음식의 인식 품질이 같다고 일반화하지 않았습니다.

이후 미등록 음식의 영양소를 함께 추정하도록 응답이 길어지면서 문서상 비용은 약 0.92원으로 늘었습니다. 따라서 `0.68원`은 **분석 구조를 결정하던 당시 저해상도 전환 실측값**으로 구분해 기록했습니다.

## 분석기가 바뀌어도 이후 계산은 그대로 사용합니다

GPT Vision과 로컬 분석 경로는 사진을 해석하는 방식이 다릅니다. 로컬 경로에서는 마스크의 픽셀 면적을 얻을 수 있지만, GPT Vision에서는 음식별 면적 비중을 반환하도록 했습니다.

그래서 두 방식의 출력 경계를 **음식명·신뢰도·면적 비중** `0~1`로 통일했습니다. 픽셀 수가 아닌 비율을 사용해 **해상도가 달라도 같은 형식으로 다룰 수 있도록 했고**, 이후 영양 테이블 조회와 양 계산은 분석 방식과 관계없이 같은 코드가 담당합니다.
...
현재는 `ANALYZER_BACKEND` 설정으로 `mock`, `openai`, `gemini`, `local` 분석기를 선택할 수 있습니다. 초기 운영에서는 GPT Vision을 사용하지만, 이후 로컬 분석 모델을 충분히 검증하면 **사진 분석 부분만 교체하고 영양 계산과 앱 응답 구조는 그대로 유지**할 수 있습니다.
